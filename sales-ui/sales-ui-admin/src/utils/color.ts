...
export const getReadableTagStyle = (color?: string) => {
  if (!color) return {}
  const normalizedColor = normalizeHexColor(color)
  if (!normalizedColor) return {}

  return {
    backgroundColor: mix(normalizedColor, '#ffffff', 0.1),
    borderColor: mix(normalizedColor, '#ffffff', 0.32),
    color: mix(normalizedColor, '#111827', 0.55),
    fontWeight: 500
  }
}
