package cn.ai.sales.module.infra.service.codegen.inner;
...
import cn.ai.sales.framework.apilog.core.annotation.ApiAccessLog;
import cn.ai.sales.framework.apilog.core.enums.OperateTypeEnum;
...
import cn.ai.sales.framework.common.util.collection.CollectionUtils;
import cn.ai.sales.framework.common.util.date.DateUtils;
import cn.ai.sales.framework.common.util.date.LocalDateTimeUtils;
import cn.ai.sales.framework.common.util.object.BeanUtils;
import cn.ai.sales.framework.common.util.object.ObjectUtils;
import cn.ai.sales.framework.common.util.string.StrUtils;
import cn.ai.sales.framework.excel.core.annotations.DictFormat;
import cn.ai.sales.framework.excel.core.convert.DictConvert;
import cn.ai.sales.framework.excel.core.util.ExcelUtils;
import cn.ai.sales.framework.mybatis.core.dataobject.BaseDO;
...
import cn.ai.sales.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import cn.ai.sales.module.infra.dal.dataobject.codegen.CodegenTableDO;
import cn.ai.sales.module.infra.enums.codegen.CodegenFrontTypeEnum;
import cn.ai.sales.module.infra.enums.codegen.CodegenSceneEnum;
import cn.ai.sales.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import cn.ai.sales.module.infra.enums.codegen.CodegenVOTypeEnum;
import cn.ai.sales.module.infra.framework.codegen.config.CodegenProperties;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 sales-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 sales-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("cn.ai.sales.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/sales-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/sales-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "sales-module-${table.moduleName}/" + // 顶级模块
                "sales-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "sales-module-${table.moduleName}/" + // 顶级模块
                "sales-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "sales-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "sales-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "sales-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "sales-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
